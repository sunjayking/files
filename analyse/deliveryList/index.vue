  ...
  <el-card class="box-card visit-list-container">
    ...
    <div class="list-header">
      ...
      <el-form-item label="Pid" prop="pid" :error="errorList.source.isError?errorList.source.errMsg:''" v-if="!!ruleForm.cid.cid && !!ruleForm.qid.qid">
        <el-select v-model="ruleForm.pid" placeholder="请选择Pid" value-key="pid" filterable @change="pidChange">
          <el-option :label="i.pid" :value="i" v-for="i in pidList" :key="i.pid"></el-option>
        </el-select>
      </el-form-item>
      ...
      <div class="list-container-wrapper">
        ...
        <div class="list-table hidden-xs-only">
          <el-table  :data="orderList.curList" border height="100%" stripe>
            <el-table-column prop="time" label="收货时间"></el-table-column>
            <el-table-column prop="collect_quantity" label="收货订单数"></el-table-column>
            <el-table-column prop="collect_test" label="累计专业检测订单数"></el-table-column>
            <el-table-column prop="collect_negotiated" label="累计正议价订单数"></el-table-column>
            <el-table-column prop="collect_waivers" label="累计豁免的议价订单数"></el-table-column>
            <el-table-column prop="collect_exempted" :formatter="formatterPrice" label="累计豁免的议价金额"></el-table-column>
            <el-table-column prop="collect_return" label="累计退货订单数"></el-table-column>
            <el-table-column prop="recycle_amount" label="累计成交量"></el-table-column>
            <el-table-column prop="closing" label="收货-成交率"></el-table-column>
            <el-table-column prop="recycle_money" :formatter="formatterPrice" label="累计成交金额"></el-table-column>
            <el-table-column prop="recycle_valuation" :formatter="formatterPrice" label="累计成交的估价金额"></el-table-column>
            <el-table-column prop="negotiated_order" label="议价中订单数"></el-table-column>
            <el-table-column prop="pending_order" label="代付款订单数"></el-table-column>
            <el-table-column prop="return_order" label="待退货订单数"></el-table-column>
          </el-table>
        </div>
        ...
        <div class="list-pagination">
          <el-button  icon="el-icon-download" class="" @click="download" type="primary" style="margin-bottom:20px;margin-left:10px;">下载模板</el-button>      
          <el-pagination
    ...
    methods: {
      ...
      getPidList(){
        ...
        axios.post('/info', megerParam('getPidList', params)).then(d => {
          this.pidList = [ {pid: 0} ].concat(d.list);
          return d
        }).catch(() => {
      ...
      download () {
        ...
        this.$confirm('确认导出吗?', '提示', {
            ...
            delete data['page'];
            delete data['length'];
            return this.$store.dispatch('exportReports2',data)
          }).then(data => {
            this.$message({
              type: 'success',
              message: '导出成功'
